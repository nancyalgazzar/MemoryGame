...
let secondImage: HTMLImageElement
let currentState: states = states.noCardSelected
...
async function flipFirst() {
    ...
    try {
        ...
        if (firstIndex >= 0 && secondIndex >= 0 && firstIndex != secondIndex && arr[firstIndex] == arr[secondIndex]) {
            ...
            if (firstIndex != -1) firstImage.src = DEFAULTIMG
            if (secondIndex != -1) secondImage.src = DEFAULTIMG
        }
        firstIndex = -1
        secondIndex = -1
...
function createDelayPromise(time: number) {
